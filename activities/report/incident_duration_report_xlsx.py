...
class IncidentDurationReportXlsx(models.AbstractModel):
    ...
    def generate_xlsx_report(self, workbook, data, obj):
        ...
        sheet.write(0, 4, 'AUTEUR', th_format)
        sheet.write(0, 5, 'SOCIÉTÉ', th_format)
        sheet.write(0, 6, 'BADGE', th_format)
        sheet.write(0, 7, 'VICTIME', th_format)
        sheet.write(0, 8, 'SOCIÉTÉ', th_format)
        sheet.write(0, 9, 'BADGE', th_format)
        ...
        for incident_id in incident_ids:
            ...
            sheet.write(i, 4, incident_id.auteur if incident_id.auteur else "", td_format)
            sheet.write(i, 5, incident_id.auteur_company if incident_id.auteur_company else "", td_format)
            sheet.write(i, 6, incident_id.auteur_badge if incident_id.auteur_badge else "", td_format)
            sheet.write(i, 7, incident_id.victime if incident_id.victime else "", td_format)
            sheet.write(i, 8, incident_id.victime_company if incident_id.victime_company else "", td_format)
            sheet.write(i, 9, incident_id.victime_badge if incident_id.victime_badge else "", td_format)
